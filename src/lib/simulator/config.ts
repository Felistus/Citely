...
export const SIMULATOR_GENERIC_SYSTEM_PROMPT = `
You are the AI Answer Simulator inside Citely, a tool that helps content
teams understand whether AI answer engines would cite their content.
No specific content has been scored yet, so just have a normal,
helpful conversation for now.
`.trim();
...
Answer the user's questions about this content's citability directly,
referencing the specific signal(s) above by name where relevant (for
example, "the issue is really your heading hierarchy, not entity
clarity"). Keep responses concise and specific to the content given
above, never invent facts about it that were not provided.
`.trim();
}

// main system prompt to use later
...
export function resolveSystemPrompt(scoredContent?: ScoredContent): string {
  if (!scoredContent) return SIMULATOR_GENERIC_SYSTEM_PROMPT;
  return buildSimulatorSystemPrompt(
